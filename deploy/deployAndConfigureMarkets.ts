...
const func = async ({ deployments, getNamedAccounts, gmx }: HardhatRuntimeEnvironment) => {
  ...
  async function setMinMarketAmount(marketToken: string) {
    const key = keys.minMarketTokensForFirstDeposit(marketToken);
    await setUintIfDifferent(key, expandDecimals(1, 10));
  }
  ...
  for (const marketConfig of markets) {
    ...
    await setMinMarketAmount(marketToken)
